...
export async function GetTournamentList(MaxResults: number, Page: number, AccessToken: string) {
  try {
    console.log(`[GetTournamentList] Starting - MaxResults: ${MaxResults}, Page: ${Page}`);
    ...
    console.log(`[GetTournamentList] Found ${FoundTournaments.length} tournaments, Total: ${TotalCount}`);
    ...
    for (let Tour of FoundTournaments) {
      try {
        ...
        const PrizeSetting = [
          {
            reward: Tour.Prizes && Array.isArray(Tour.Prizes) && Tour.Prizes.length > 0
              ? Tour.Prizes.map((prize) => ({
                  "@position": prize?.position?.toString() || "1",
                  item: [
                    {
                      "@amount": prize?.amount?.toString() || "0",
                      "@type": "10",
